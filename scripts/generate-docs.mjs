...
function cliPage(truth) {
  ...
  return page({
    ...
    content: `<!-- generated from src/cli/command-registry.ts -->${hero('04 · GENERATED CLI', `CLI ${truth.cliVersion} 全命令`, '本页命令、摘要、输出和示例与 /swob skill 使用同一个 command registry。改代码不重新生成，测试会失败。', `<div class="hero-badges"><span>${truth.cliCommands.length} 条命令</span><span>机器输出优先 JSON</span></div>`)}<section><h2>机器接口约定</h2><ul class="checks"><li><code>--json</code> 时 stdout 只包含合法 JSON，诊断写 stderr。</li><li><code>resolve</code>、<code>where</code>、<code>transcript status</code> 只读轻量 manifest/registry 和文件时间戳，不读取会话正文、不依赖 GUI 搜索索引。</li><li>完整 ID 以 Library manifest 为存在性事实；lineage 只增强解析。短前缀歧义退出 2，扫描不完整时不会猜测唯一结果。</li><li><code>show --format=jsonl</code> 每行一个独立 JSON 事件。</li><li>批量 move/rename 整批先校验再提交；<code>undo</code> 只撤销最近一次组织事务。</li><li>token 汇总的 <code>input_plus_output</code> 不包含 cache creation/read。</li></ul><table class="compact"><thead><tr><th>退出码</th><th>含义</th></tr></thead><tbody>${exits}</tbody></table></section><section><h2>定位、诊断与局部恢复</h2><p><code>where</code> 返回 package、manifest、transcript、backup、source 路径及存在性；<code>transcript status</code> 返回四个 mtime、lag、stale 与阻塞原因。<code>transcript rebuild &lt;id&gt;</code> 只加载并修改唯一目标包；<code>doctor locks/library</code> 只读报告 writer 与全库健康状态，不删除锁。</p><div class="callout warning"><strong>隐私与 fail-closed</strong><p>这些命令不输出会话正文、密钥、writer nonce、完整 device/boot/process 指纹。损坏 manifest、重复身份、iCloud placeholder 和 writer busy 都返回稳定机器错误码，不以猜测继续写入。</p></div></section><section><h2>命令参考</h2><div class="command-list">${commandRows}</div></section>`
  })
}
