 ...
 * 2. Same project has nodes in cache → near project centroid
 * 3. Same source has nodes in cache → near source centroid
 * 4. Fallback → hash-derived sector coordinates
 */
export function computeNewNodePosition(
  ...
  // Deterministic small offsets from hash
  ...
  // 2. Near project centroid
  if (project) {
    const projectPositions = collectGroupPositions(ctx, (k, n) => n.project === project)
    if (projectPositions.length > 0) {
      const centroid = computeCentroid(projectPositions)
      return {
  ...
  const sourcePositions = collectGroupPositions(ctx, (k, n) => n.source === source)
  ...
  // 4. Fallback — sector from source index + hash jitter
