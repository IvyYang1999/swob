...
test('Grok composite source opens from sidebar and renders compact history, tools, reasoning and usage', async ({}, testInfo) => {
  ...
  try {
    ...
    await currentPrompt.scrollIntoViewIfNeeded()
    await expect(currentPrompt).toBeVisible()
    const currentPromptBox = await currentPrompt.boundingBox()
    const viewport = await page.evaluate(() => ({ width: window.innerWidth, height: window.innerHeight }))
    expect(currentPromptBox).not.toBeNull()
    expect(currentPromptBox!.width).toBeGreaterThan(0)
    expect(currentPromptBox!.height).toBeGreaterThan(0)
    expect(currentPromptBox!.x).toBeGreaterThanOrEqual(0)
    expect(currentPromptBox!.y).toBeGreaterThanOrEqual(0)
    expect(currentPromptBox!.x + currentPromptBox!.width).toBeLessThanOrEqual(viewport.width)
    expect(currentPromptBox!.y + currentPromptBox!.height).toBeLessThanOrEqual(viewport.height)
