...
test('Pi canonical provider renders transcript, thinking, tools, and usage in the product UI', async ({}, testInfo) => {
  ...
  try {
    ...
    await expect(chatScroll.getByText('Find the synthetic bridge marker.', { exact: true })).toBeVisible()
    await expect(chatScroll.getByText(/I will inspect the generated sample/)).toBeVisible()
    await expect(chatScroll.getByText(/Inspect the sample fixture without touching private data/)).toBeVisible()
    await expect(chatScroll.getByText('read', { exact: true })).toBeVisible()
    await expect(chatScroll.locator('[title*="Input: 150 tokens"]')).toContainText('180 tok')
    await expect(chatScroll.locator('[title*="Input: 80 tokens"]')).toContainText('98 tok')
    ...
    await expect(chatScroll.getByText(/\/workspace\/synthetic-pi\/README\.md/)).toBeVisible()
    ...
    const toolInput = chatScroll.getByText(/\/workspace\/synthetic-pi\/README\.md/)
